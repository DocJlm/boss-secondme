...
export function EmployerCard({
  ...
  const [selectedJobId, setSelectedJobId] = useState<string | null>(
    employer.jobs.length > 0 ? employer.jobs[0].id : null
  );
  const [isJobsExpanded, setIsJobsExpanded] = useState(false);

  const handleMatch = async () => {
    // 启动匹配对话
    // 注意：匹配页面的 userId 参数应该是候选人（当前用户）的 ID
    if (selectedJobId) {
      window.location.href = `/match/${candidateUserId}?jobId=${selectedJobId}`;
    }
  };
      ...
      >
        ...
        >
          ...
          <div className="text-center mb-4">
            ...
            {(employer.user.name || employer.name) && (
              <p className="text-slate-600 mb-1">{employer.user.name || employer.name}</p>
            )}
            {employer.title && (
              <p className="text-sm text-slate-500">{employer.title}</p>
            )}
          ...
          {employer.jobs.length > 0 && (
            <div className="mb-4">
              <div className="flex items-center justify-between mb-2">
                <p className="text-sm font-medium text-slate-700">招聘职位：</p>
                {employer.jobs.length > 1 && (
                  <button
              ...
              {employer.jobs.length === 1 ? (
                <div className="px-3 py-2 rounded-lg bg-orange-50 border border-orange-200">
                  <p className="text-sm font-medium text-orange-900">{employer.jobs[0].title}</p>
                  {employer.jobs[0].city && (
                    <p className="text-xs text-orange-700 mt-1">📍 {employer.jobs[0].city}</p>
                  )}
                  {(employer.jobs[0].salaryMin || employer.jobs[0].salaryMax) && (
                    <p className="text-xs text-orange-700">
                      💰 {employer.jobs[0].salaryMin || "面议"}
                      {employer.jobs[0].salaryMax && employer.jobs[0].salaryMin ? `-${employer.jobs[0].salaryMax}` : ""} {employer.jobs[0].salaryCurrency || "元"}
                    </p>
                  )}
                </div>
              ) : (
                  ...
                  {selectedJobId && (
                    <div className="px-3 py-2 rounded-lg bg-orange-50 border-2 border-orange-500">
                      <p className="text-sm font-medium text-orange-900">
                        {employer.jobs.find((j) => j.id === selectedJobId)?.title}
                      </p>
                      {employer.jobs.find((j) => j.id === selectedJobId)?.city && (
                        <p className="text-xs text-orange-700 mt-1">
                          📍 {employer.jobs.find((j) => j.id === selectedJobId)?.city}
                        </p>
                      )}
                      {(employer.jobs.find((j) => j.id === selectedJobId)?.salaryMin ||
                        employer.jobs.find((j) => j.id === selectedJobId)?.salaryMax) && (
                        <p className="text-xs text-orange-700">
                          💰{" "}
                          {employer.jobs.find((j) => j.id === selectedJobId)?.salaryMin || "面议"}
                          {employer.jobs.find((j) => j.id === selectedJobId)?.salaryMax &&
                          employer.jobs.find((j) => j.id === selectedJobId)?.salaryMin
                            ? `-${employer.jobs.find((j) => j.id === selectedJobId)?.salaryMax}`
                            : ""}{" "}
                          {employer.jobs.find((j) => j.id === selectedJobId)?.salaryCurrency || "元"}
                        </p>
                      )}
                    </div>
                  )}
        ...
        >
          ...
          <div className="flex-1 overflow-y-auto">
            <h3 className="text-lg font-semibold text-slate-900 mb-3">公司简介</h3>
            {employer.company?.intro ? (
              <p className="text-sm text-slate-700 whitespace-pre-wrap">
                {employer.company.intro}
              </p>
            ) : (
              <p className="text-sm text-slate-500">暂无公司简介</p>
            )}
          ...
          <div className="mt-4">
            <button
              onClick={handleMatch}
              className="w-full px-4 py-3 rounded-xl text-sm font-medium bg-orange-600 text-white hover:bg-orange-700 transition-colors shadow-lg"
            >
              开始匹配
            </button>
          </div>
